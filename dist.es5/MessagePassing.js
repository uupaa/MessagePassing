...
var MessagePassing = /** @class */ (function () {
    ...
    MessagePassing.prototype.to = function () {
        ...
        var to = new Set(subscribers.length ? subscribers.concat() // broadcast
...
var Message = /** @class */ (function () {
    ...
    Message.prototype.send = function (selector, options) {
        if (options === void 0) { options = undefined; }
        var to = Array.from(this._to.values());
        var result = new MessageResult();
        try {
            for (var i = 0, iz = to.length; i < iz; ++i) {
                var subscriber = to[i];
                if (subscriber) {
                    var selectors = this._selectors.get(subscriber); // [ "ping", ... ]
                    if (selectors && selectors.includes(selector)) {
                        var r = subscriber.onmessage(selector, options);
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        _a.label = 2;
                    case 2:
                        if (!(i < iz)) return [3 /*break*/, 5];
                        subscriber = to[i];
                        if (!subscriber) return [3 /*break*/, 4];
                        selectors = this._selectors.get(subscriber);
                        if (!(selectors && selectors.includes(selector))) return [3 /*break*/, 4];
                        return [4 /*yield*/, subscriber.onmessage(selector, options)];
                    case 3:
                        r = _a.sent();
                        result.set(subscriber, r);
                        return [3 /*break*/, 4];
